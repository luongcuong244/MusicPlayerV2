...
//        )
    ...
    fun isFavoriteSong(songId: String, apiCallback: ApiCallback<Boolean>) {
        getFavouriteSongs(
            object : ApiCallback<List<Song>> {
                override fun onSuccess(data: List<Song>?) {
                    if (data != null) {
                        data.find { it.id == songId }?.let {
                            return apiCallback.onSuccess(true)
                        }
                    }
                    return apiCallback.onSuccess(false)
                }

                override fun onFailure(message: String) {
                    return apiCallback.onFailure(message)
                }
            }
        )
    }
    ...
    fun addFavouriteSong(song: Song, apiCallback: ApiCallback<Void>) {
        songApi.addFavouriteSong(AddFavouriteSongRequest(CurrentUser.getUser()!!.id, song.id)).enqueue(
            object : Callback<AddFavouriteSongResponse> {
                override fun onResponse(call: Call<AddFavouriteSongResponse>, response: Response<AddFavouriteSongResponse>) {
                ...
                override fun onFailure(call: Call<AddFavouriteSongResponse>, t: Throwable) {
                    apiCallback.onFailure(t.message ?: "Unknown error")
                }
            }
        )
    ...
    fun removeFavouriteSong(song: Song, apiCallback: ApiCallback<Void>) {
        songApi.removeFavouriteSong(RemoveFavouriteSongRequest(CurrentUser.getUser()!!.id, song.id)).enqueue(
            object : Callback<RemoveFavouriteSongResponse> {
                override fun onResponse(call: Call<RemoveFavouriteSongResponse>, response: Response<RemoveFavouriteSongResponse>) {
                    apiCallback.onSuccess(null)
                }

                override fun onFailure(call: Call<RemoveFavouriteSongResponse>, t: Throwable) {
                    apiCallback.onFailure(t.message ?: "Unknown error")
                }
            }
        )
